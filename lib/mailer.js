...
export async function sendContactEmail({ name, email, message }) {
  ...
  const html = `
    ...
    <head>
      ...
      <style>
        ...
        .header h1 { color: #fff; margin: 0; font-size: 1.6rem; font-weight: 700; }
        .body { padding: 36px; }
        .body p { color: #4a2155; line-height: 1.7; margin: 0 0 10px; }
        .quote { background: rgba(217,168,232,0.15); border-left: 3px solid #D41479; padding: 14px 18px; border-radius: 8px; font-size: 0.95rem; color: #1A0A1D; font-style: italic; white-space: pre-wrap; margin: 20px 0; }
        .footer { text-align: center; color: #aaa; font-size: 0.8rem; padding: 24px 36px; }
      </style>
    </head>
    <body>
  ...
  await transporter.sendMail({
    from: `"Tidy Mimo Website" <${process.env.SMTP_USER}>`,
    to: 'info@tidymimo.com',
    replyTo: email,
