...
export default function AdminDashboard() {
  ...
  const [currentTab, setCurrentTab] = useState('products'); // 'products' or 'inquiries'
  ...
  }, [loadProducts, loadMessages, currentTab]);
  ...
  const handleToggleRead = async (id, isRead) => {
    try {
      await axios.patch(`/api/admin/messages/${id}`, { isRead });
  ...
  return (
    <div className="flex flex-col md:flex-row md:max-h-screen min-h-screen bg-[#F9F0FB] font-[Inter]">
      ...
      <aside className="w-full md:w-[240px] md:min-h-screen bg-gradient-to-b from-[#3A0D45] to-[#1A0822] flex flex-col shrink-0 md:sticky top-0 z-40">
        ...
        `}>
          <div className="p-4 md:p-3 flex flex-col gap-1.5">
             <button 
                onClick={() => { setCurrentTab('products'); setMobileMenuOpen(false); }}
                className={`
                  w-full text-left px-4 py-3 rounded-xl border-none cursor-pointer text-sm font-bold transition-all duration-200
                  ${currentTab === 'products' 
                    ? 'bg-gradient-to-r from-[#D41479] to-[#6C2A79] text-white shadow-lg shadow-pink-500/20' 
              ...
              <button 
                onClick={() => { setCurrentTab('inquiries'); setMobileMenuOpen(false); }}
                className={`
                  w-full text-left px-4 py-3 rounded-xl border-none cursor-pointer text-sm font-bold transition-all duration-200 relative
              ...
              >
                ...
                {messages.filter(m => !m.isRead).length > 0 && (
                  <span className="absolute top-2 right-2 w-2 h-2 bg-primary-pink rounded-full ring-4 ring-[#3A0D45]/30"></span>
                )}
              </button>
          </div>

          {currentTab === 'products' && (
            <nav className="flex-1 p-4 md:p-3 flex flex-col gap-1.5 border-t border-[#D9A8E8]/10 pt-4">
              <span className="px-4 text-[0.65rem] font-black text-[#FCE8F4]/30 uppercase tracking-widest mb-2">Categories</span>
              {['all', 'household', 'industrial', 'carcare', 'hotels'].map(cat => (
                <button 
                  ...
                  className={`
                    ...
                    ${filter === cat 
                      ? 'bg-white/10 text-[#FCE8F4]' 
                      : 'bg-transparent text-[#FCE8F4]/65 hover:bg-white/5'
                    }
                  `}
                >
                  {cat === 'all' ? 'Show All' : CATEGORY_LABELS[cat]}
        ...
        {currentTab === 'products' ? (
          <>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 md:gap-6 pb-12">
          ...
          <div className="animate-fadeIn">
            {/* Inquiries Header */}
            <div className="mb-8">
              <h1 className="m-0 text-2xl md:text-3xl font-bold text-[#1A0A1D] tracking-tight">Website Inquiries</h1>
              <p className="m-0 mt-1.5 text-[#1A0A1D]/60 text-sm">
                ...
                {messages.map(msg => (
                  <div 
                    ...
                    className={`
                      ...
                      ${msg.isRead ? 'border-[#D9A8E8]/30 opacity-75' : 'border-[#D41479]/40 shadow-[0_4px_20px_rgba(212,20,121,0.06)] scale-[1.01]'}
                    `}
                  >
                    <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-4">
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          <h3 className="m-0 text-lg font-bold text-[#1A0A1D]">{msg.name}</h3>
                          {!msg.isRead && (
                            <span className="bg-primary-pink text-white text-[10px] uppercase font-black px-2 py-0.5 rounded-md">New</span>
                          )}
                        </div>
                        <a href={`mailto:${msg.email}`} className="text-[#D41479] font-medium text-sm hover:underline">{msg.email}</a>
                      </div>
                      <div className="text-[#1A0A1D]/40 text-xs font-bold uppercase tracking-wider">
                        {new Date(msg.createdAt).toLocaleDateString()} at {new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                      </div>
                    </div>
                    
                    <div className="bg-[#F9F0FB] p-4 rounded-xl text-[#3A0D45] text-sm leading-relaxed mb-4 whitespace-pre-wrap">
                      {msg.message}
                    </div>
                    ...
                    <div className="flex gap-2">
                       <button 
                        ...
                        className={`
                          ...
                          ${msg.isRead 
                            ? 'bg-[#6C2A79]/10 text-[#6C2A79] hover:bg-[#6C2A79]/20' 
                            : 'bg-[#D41479] text-white hover:bg-[#D41479]/90'
                          }
                        `}
                      >
                      ...
                      <button 
                        ...
                        className="px-4 py-2 rounded-lg bg-red-500/10 text-red-500 text-xs font-bold hover:bg-red-500 hover:text-white transition-colors disabled:opacity-50"
                      >
                        {deleting === msg.id ? 'Deleting...' : 'Delete Inquiry'}
                      </button>
                    </div>
                  </div>
                ))}
